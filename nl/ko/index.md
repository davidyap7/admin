...
| 수행할 수 있는 작업| 세부사항|    
|----------------|---------|
|시스템 사용량 모니터링| **관리 &gt; 사용량**을 클릭하십시오. 시스템 정보를 확인하고, CPU 사용량을 모니터링하고, 사용량을 계획하여 비즈니스에 가장 최선의 결정을 내리십시오. [사용량 정보 보기](/docs/admin/index.html#oc_resource)를 참조하십시오.|
 ...
 * 이메일 옵션을 선택하면 지정하는 이메일 주소에 알림이 전송됩니다. 인시던트, 유지보수 업데이트 또는 임계값의 알림을 선택할 수 있습니다. 초기 이메일 알림이 전송됩니다. 그런 다음 이벤트가 변경되는 경우, 변경이 수행될 때마다 해당 변경을 포함하는 다른 알림이 전송됩니다.  
...
| 수신| 이메일 알림의 수신인에 대해 쉼표로 구분된 목록을 사용하여 이메일 주소를 입력하십시오. 이메일에서 다른 사용자를 복사하려면 "참조" 또는 "숨은 참조" 옵션을 펼치십시오. 필수 필드입니다. |
...
**참고**: 시작하려면 [사전 승인된 업데이트 기간 설정](index.html#preapprovedmaintenance)에 대한 다음 섹션을 참조하십시오. IBM이 사용자 환경에 대한 유지보수를 스케줄하려면 해당 기간을 설정해야 합니다.
...
5. 선택사항: 반복 업데이트 기간을 스케줄링할 배치의 선호 시간으로 설정하려면 **선호사항으로 표시**를 선택하십시오. 가능한 경우 선호 기간에 우선순위를 둡니다. 
6. **제출**을 클릭하십시오.
...
가동이 중지되지 않는 유지보수 업데이트에 사용자 환경을 사용할 수 없는, 특정 블랙아웃 기간을 설정하도록 선택할 수 있습니다. 예를 들어, 트래픽이 높은 주말 또는 공휴일에는 유지보수를 적용하지 않도록 선택하여 사용자가 애플리케이션을 사용할 수 있도록 할 수 있습니다.
...
블랙아웃 날짜는 IBM에서 승인해야 하며, 승인을 받는 데 걸리는 시간은 경우에 따라 다릅니다. 요청한 블랙아웃 날짜가 승인되면 IBM에서는 사용 불가능한 기간 동안의 현재 스케줄링된 기존 업데이트를 모두 취소합니다. IBM에서는 이러한 업데이트에 대한 새 레코드를 작성한 다음 승인된 블랙아웃 날짜를 제외한 날짜로 스케줄링합니다.
...
사전 승인된 업데이트 기간 설정 및 유지보수에 사용할 특정 블랙아웃 날짜 설정에 대한 자세한 정보는 [유지보수 업데이트](index.html#oc_schedulemaintenance)를 참조하십시오. 
...
실제 및 예약 메모리 또는 시간 경과에 따른 디스크 사용량에 대한 자세한 내용을 보려면 **히스토리**를 클릭하십시오. 최근 48시간, 최근 30일 또는 최근 5개월 동안의 사용량을 볼 수 있습니다. 최근 48시간에 대해 표시되는 점은 시간당 한 번 수집되는 실제 데이터를 나타냅니다. 최근 30일에 대해 표시되는 점은 각각의 날에 대한 평균 값을 나타냅니다. 최근 5개월에 대해 표시되는 점은 각 달에 대한 평균 값을 나타냅니다. 최근 30일 및 최근 5개월 보기는 차트의 맨 위에 있는 단일 선택 단추를 전환하여 각 요일 또는 월의 최대값을 표시할 수 있습니다. 히스토리 사용량 보기는 선택한 시간 동안의 메모리 또는 디스크 사용량 그래프를 표시합니다.   
	...
	<dt><strong>예약</strong></dt>
	<dd>예약된 선은 환경에서 실행 중인 모든 애플리케이션에서 현재 총괄적으로 예약한 메모리 또는 디스크 공간을 표시합니다. </dd>
...
사용 가능한 히스토리 사용량 보기 각각의 경우, 그래프를 렌더링하려면 두 개 이상의 데이터 점이 필요합니다. 최근 48시간 동안에 두 시간, 최근 30일 동안에 2일, 최근 5개월 동안에 2개월 이상에 대한 데이터와 동일합니다. 30일 및 5개월 보기의 경우, 부분적으로 완료된 평균 또는 최대값이 현재 일 또는 월에 대해 리턴되며 더 많은 데이터가 수집됨에 따라 조정됩니다. 
...
**참고**: 일부 환경에서는 현재 보고서 요청 기능을 사용하지 못할 수 있습니다. 
...
![카탈로그에서 플랜 메타데이터 세부사항.](images/plan_metadata.png "Bluemix 카탈로그 플랜 메타데이터 값 보기")
...
차트에는 다음 유형의 메모리 사용량이 표시됩니다.
	...
	<dd>모든 조직의 배치된 모든 앱을 위해 예약된 메모리의 합계입니다. 예약된 할당량의 합계는 환경의 실제 메모리 한계를 초과할 수 있습니다. 예를 들어, 실제 메모리 한계가 16GB인 경우, 총 5개의 서로 다른 애플리케이션에 대해 각각 4GB를 예약할 수 있습니다. 예약된 할당량이 실제 메모리 한계를 초과합니다. 그러나 많은 경우에 애플리케이션은 각 애플리케이션에 대해 개별적으로 예약된 할당량을 전부 사용하지 않을 수 있습니다. 또한 모든 애플리케이션이 동시에 예약 메모리 할당량을 전부 사용하지 않을 수 있습니다.</dd>
	<dt><strong>예약 한계</strong></dt>
...
<p>컨테이너에 대한 자세한 정보는 [IBM 컨테이너 정보](/docs/containers/container_ov.html)를 참조하십시오. 컨테이너 할당량에 대한 자세한 정보는 [할당량 및 {{site.data.keyword.Bluemix_notm}} 계정](/docs/containers/container_planning_org_ov.html#container_planning_quota)을 참조하십시오. </p>
...
* 단일 사용자를 추가합니다.  **수퍼유저** 권한 또는 **쓰기** 액세스 권한이 포함된 **사용자** 권한이 있으면 사용자를 추가할 수 있습니다. 
...
* LDAP 디렉토리에서 사용자 그룹을 추가합니다. **수퍼유저** 권한 또는 **쓰기** 액세스 권한이 포함된 **사용자** 권한이 있으면 사용자를 추가할 수 있습니다. 
...
| **사용자 권한** | **설명** |       
|-----------------|-------------------|
| 수퍼유저| **수퍼유저** 권한이 **설정**으로 지정된 사용자는 다른 사용자의 권한을 편집할 수 있습니다. 권한이 설정되어 있으면 기타 모든 권한에 대한 전체 액세스 권한을 자동으로 사용 가능하게 합니다. 이 표에서 각 권한에 대해 요약된 태스크 이외에, 알림 구독을 설정하여 유지보수 또는 인시던트에 대해 직접 경보를 받고 유지보수를 스케줄하며 콘솔 컴포넌트에 대한 확인 검사를 실행하고 환경의 조직과 영역을 작성할 수도 있습니다. 이 권한은 관리 콘솔의 관리자(admin) 역할과 동일합니다. |
| 기본 액세스| **기본 액세스** 권한이 **설정**으로 지정된 사용자는 {{site.data.keyword.Bluemix_notm}} 사용자 인터페이스에서 관리 페이지 옵션을 볼 수 있습니다. 권한이 사용으로 설정된 사용자는 [시스템 정보](#oc_system) 및 [리소스 사용량](#oc_resource) 타일에 액세스할 수 있습니다. 이 권한이 없으면 사용자가 관리 메뉴 옵션을 보거나 이에 액세스할 수 없습니다.이 권한은 관리 콘솔의 관리자(admin) 역할과 동일합니다. 이 권한은 이전에 사용한 관리 콘솔의 로그인 권한과 동일합니다.|
...
관리 콘솔에 로그인하기 위해 `https://console.<region>.bluemix.net/login` 엔드포인트에서 기본 액세스 인증을 사용할 수 있습니다. 서버에서 사용자 세션의 쿠키가 리턴됩니다. 관리 콘솔의 모든 작업에
이 쿠키를 사용합니다. 
...
* 사용자의 이름과 성. [사용자 목록](index.html#listingusr)에 나와 있는
`"first_name"` 및 `"last_name"`을 제공하십시오.
* 이메일 주소 및 사용자 ID: 이메일 주소와 사용자 ID에 대해 [사용자 목록](index.html#listingusr)에 나와 있는 `"user_id"`를 제공하십시오. 
...
네 가지 API를 사용하여 사용자의 환경, 애플리케이션 및 조직에 대한 메트릭을 수집할 수 있습니다. 이러한 API는 사용자가 지정한 시간 동안 요청한 메트릭에 대한 데이터 점 배열을 리턴합니다. 
...
4. API 요청이 발생한 시점부터 최대 5개월 이전의 데이터가 사용 가능합니다. 

## 환경에 대한 메트릭 수집

환경 API를 사용하여 지정하는 기간 동안의 상위 레벨 환경 정보를 수집할 수 있습니다. 거의 매 시간마다 기록되는 실제 데이터 샘플 또는 그 데이터 샘플에서 계산된 통계를 볼 수 있습니다. 통계는 메모리 및 디스크 사용량에만 사용할 수 있습니다. API는 리턴된 실제 또는 통계 데이터의 시간 범위를 지정하도록 허용합니다. 예를 들어, 환경에 대해 6시간의 CPU (실제) 데이터를 요청하는 경우 요청된 6시간마다 CPU 데이터가 응답에 포함됩니다. 
...
**참고**: 다음 권한 중 하나가 이러한 엔드포인트에 액세스하는 데 필요합니다. **기본 액세스**, **사용자 읽기**, **사용자 쓰기** 또는 **수퍼유저**
...
사용자 환경의 메모리 및 디스크 사용량에 대한 메트릭 데이터에서 계산된 통계를 볼 수 있습니다. 리턴된 통계는 각 데이터 그룹화에서 데이터 샘플의 평균, 최대 및 최소값입니다. 데이터는 각각의 날에 대해 통계가 계산된 일별로, 또는 각각의 월에 대해 통계가 계산된 월별로 그룹화할 수 있습니다.
...
다음 조회 매개변수를 사용하여 디스크 및 메모리에 대한 통계를 수집할 수 있습니다. 

<dl class="parml">
<dt class="pt dlterm">startTime</dt>
<dd class="pd">데이터가 리턴되는 가장 빠른 시점입니다. startTime을 지정하지 않으면 사용 가능한 가장 빠른 데이터 점이 포함됩니다. </dd>
<dt class="pt dlterm">endTime</dt>
<dd class="pd">데이터가 리턴되는 가장 늦은 시점입니다. endTime을 지정하지 않으면 가장 최근의 데이터 점이 사용됩니다. </dd>
<dt class="pt dlterm">averageType</dt>
<dd class="pd">통계를 계산하는 그룹화입니다. 올바른 값은 `day` 및 `month`입니다. 기본값은 day이며, 지정된 시간 범위에서 각 날짜에 대한 평균 사용량 값을 리턴합니다. 월의 값을 지정하면 각 월에 대한 평균 사용량 값을 리턴합니다.</dd>
</dl>

다음 예제는 조회 매개변수를 사용하여 일별 평균으로 사용자의 환경에 대한 메모리 통계를 수집합니다. 
...
**참고**: 다음 권한 중 하나가 이러한 엔드포인트에 액세스하는 데 필요합니다. **사용자 읽기**, **사용자 쓰기** 또는 **수퍼유저**
